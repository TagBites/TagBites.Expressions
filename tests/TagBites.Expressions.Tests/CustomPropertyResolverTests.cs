...
public class CustomPropertyResolverTests
{
    ...
    [InlineData("People.FirstOrDefault(p => p.Name == \"Alice\")?.Children?.Sum(c => c.Age) ?? 0", 57)]
    ...
    [InlineData("People.Where(p => p.Children != null && p.Children.Any(c => c.Age < 18)).Count()", 1)]
    ...
    [InlineData("People.SelectMany(p => p.Children ?? new List<DynamicRecord>()).GroupBy(c => c.Age > 18).Select(g => g.Count()).Sum()", 2)]
